...
async def answer_all(state: QState) -> dict[str, Any]:
    ...
    async def _one(item: dict) -> dict:
        ...
        log.info("sqr.answered", id=item["id"], conf=ans.get("confidence"), review=ans.get("needs_review"))
        return ans

    answered = await asyncio.gather(*[_one(i) for i in items])
    review_count = sum(1 for a in answered if a.get("needs_review"))
    avg_conf = round(sum(a.get("confidence", 0) for a in answered) / max(len(answered), 1), 3)
    return {
        "answers": answered,
        "summary": {
            "total": len(answered),
        ...
        "events": [_event("answered_all", total=len(answered), needs_review=review_count, avg=avg_conf)],
    }
...
async def write_output(state: QState) -> dict[str, Any]:
    ...
    return {"output_path": str(out_path), "events": [_event("written", path=str(out_path))]}
